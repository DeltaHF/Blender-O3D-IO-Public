...
class ShaderImageTextureWrapper():
    ...
    def __init__(self, owner_shader: ShaderWrapper, node_dst, socket_dst, grid_row_diff=0,
                 use_alpha=False, colorspace_is_data=..., colorspace_name=..., is_bump_map=False):
    ...
    def image_set(self, image):
        ...
        if self.is_bump_map:
            ...
            node_bump = tree.nodes.new(type='ShaderNodeBump')
            self.owner_shader._grid_to_location(
                -1, 0 + self.grid_row_diff,
                dst_node=node_bump, ref_node=self.node_dst,
            )

            tree.links.new(self.node_image.outputs["Alpha" if self.use_alpha else "Color"], node_bump.inputs["Height"])
            tree.links.new(node_bump.outputs["Normal"], self.socket_dst)
